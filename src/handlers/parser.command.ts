...
async function removeRegexHandler(ctx: Context) {
  try {
    const { data, error } = await HSTAPI.get<Res>("/regex/list");
...
async function getRegexHandler(ctx: Context) {
  try {
    const { data, error, msg } = await HSTAPI.get<Res>("/regex/list");
    if (error) return await ctx.reply(error);
...
async function getPromptsHandler(ctx: Context) {
  try {
    const { msg, error } = await HSTAPI.get<Res>("/prompt/get");
    if (error) return await ctx.reply(error);
...
export { removeRegexHandler, getRegexHandler, addRegexHandler, getPromptsHandler, setPromptHandler };
